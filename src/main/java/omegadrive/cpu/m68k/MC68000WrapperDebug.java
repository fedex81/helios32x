...
public class MC68000WrapperDebug extends MC68000Wrapper {
    ...
    public static boolean verboseInst = false;
...
//            loopDetection();
     ...
     * 008808fe   4e71                    nop
     * 00880900   4e71                    nop
     * 00880902   6000 fffffffa               bra.w    $008808fe
     * <p>
     * 008808fe   4e71                    nop
     * 00880900   4e71                    nop
     * 00880902   6000 fffffffa               bra.w    $008808fe
    ...
    private void loopDetection() {
        if (currentPC == prev) {
            ...
            boolean loop2 =
                    currentPC == traceArray[(index + lastN + 1) % lastN].pc &&
                            prev == traceArray[(index + lastN + 2) % lastN].pc &&
                            currentPC == traceArray[(index + lastN + 3) % lastN].pc &&
                            prev == traceArray[(index + lastN + 4) % lastN].pc;
            boolean loop3 =
                    currentPC == traceArray[(index + lastN + 1) % lastN].pc &&
                            prev == traceArray[(index + lastN) % lastN].pc &&
                            currentPC == traceArray[(index + lastN + 4) % lastN].pc &&
                            prev == traceArray[(index + lastN + 3) % lastN].pc &&
                            currentPC == traceArray[(index + lastN + 7) % lastN].pc &&
                            prev == traceArray[(index + lastN + 6) % lastN].pc;
            if (loop2 || loop3) {
                loopCount++;
            } else {
                loopCount = 0;
        ...
        if (loopCount > 100000) {
            System.out.println("68k Loop detected stopping, PC: " + Integer.toHexString(currentPC));
            loopCount = 0;
            this.setStop(true);
        }
    }
